...
- [ ]: M3 (from M2 design): "honest mode" for the analyzer — determinize opponent hands (sample N hands consistent with public info), run the verified search per sample, report expected value + "lethal in X% of worlds"; panel gets a Revealed/Honest mode label. Also: click-through board preview per line step; TurnPlanner as a benchable agent tier between greedy and MCTS.
